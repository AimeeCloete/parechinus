...
w_anova4 <- aov(`weight_specific growth rate`~treatment, data = SGR4)
summary(w_anova4)
Tukey4 = TukeyHSD(w_anova4, ordered = TRUE)
Tukey4

#week8
oneway.test(`weight_specific growth rate`~diet, data = SGR8, var.equal = FALSE) #not sig
oneway.test(`weight_specific growth rate`~temperature, data = SGR8, var.equal = FALSE) #not sig
oneway.test(`weight_specific growth rate`~treatment, data = SGR8, var.equal = FALSE) #significant

oneway.test(`weight_specific growth rate`~diet+temperature, data = SGR8, var.equal = FALSE) #significant


require(rstatix)
games_howell_test(SGR8, `weight_specific growth rate`~treatment, conf.level = 0.95, detailed = FALSE)
...
summary(w_anova13) #treatment sig
...
w_anova18 <- aov(`weight_specific growth rate`~treatment, data = SGR18)
summary(w_anova18) #treatment sig
Tukey18 = TukeyHSD(w_anova18, ordered = TRUE)
Tukey18
...
w_anova23 <- aov(`weight_specific growth rate`~treatment, data = SGR23)
summary(w_anova23)
Tukey23 = TukeyHSD(w_anova23, ordered = TRUE)
Tukey23
...
#week 4
kruskal.test(`size_specific growth rate` ~ treatment, data = SGR4) #not sig
...
dunn.test(SGR4$`size_specific growth rate` , g = SGR4$temperature, altp = TRUE)
#sig
...
s_anova8<- aov(`size_specific growth rate`~treatment,data=SGR8)
summary(s_anova8) #not sig

#week 13
s_anova13<- anova_size_SGR(SGR13)
summary(s_anova13) #diet sig
s_anova13<- aov(`size_specific growth rate`~treatment,data=SGR13)
summary(s_anova13) #treatment not sig
...
s_anova18<- aov(`size_specific growth rate`~treatment,data=SGR18)
summary(s_anova18) #treatment not sig
...
SGR23 %>% group_by(treatment) %>% summarise(SGR_mean = mean(`size_specific growth rate`), SGR_se = sd(`size_specific growth rate`)/sqrt(length(`size_specific growth rate`)))
