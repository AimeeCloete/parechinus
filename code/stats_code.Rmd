...
gonad26nof <- gonad %>% filter(week == 26) %>% filter(diet!= "formulated")
...
GSI_prior_tests <- function(data){
  ...
  levene <- leveneTest(data$GSI~data$diet)
  print(shapiro)
  print(levene)
}
...
a_temp <- daily_temp %>% filter(Treatment == "Ambient")
w_temp <- daily_temp %>% filter(Treatment == "Warmed")

shapiro.test(a_temp$daytemp) #not normal
shapiro.test(w_temp$daytemp) #not normal
