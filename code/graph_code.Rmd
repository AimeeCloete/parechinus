...
somatic_growth <- somatic_growth %>%
  filter(!(week == 13 & diet == "kelp"))
```
...
new_size_growth_plot <- size_growth_dat %>%
  ...
  scale_fill_manual(values = diet_colors) + 
  labs(title = "Cape sea urchin test diameter (mm) specific growth rate (SGR) (%growth/day) per measurement interval", 
      x = "Temperature",
      y = "SGR test diameter size (mm) (%growth/day)",
      fill = "Diet")+
  theme_classic() + theme(plot.title = element_text(hjust = 0.5))
...
new_weight_growth_plot <- weight_growth_dat %>%
  ...
  facet_wrap(~ week, ncol = 5, labeller = labeller(week= week.labs)) +
  scale_fill_manual(values = diet_colors) + 
  labs(title = "Cape sea urchin wet weight (g) specific growth rate (SGR) (%growth/day) per measurement interval", 
      x = "Temperature",
      y = "SGR wet weight (g) (%growth/day)",
      fill = "Diet")+
  theme_classic() + theme(plot.title = element_text(hjust = 0.5))
...
new_gsi_plot <- GSI_dat %>%
 ...
 geom_col(position = position_dodge(width = 0.9), color = "black", width = 0.9) + geom_errorbar(aes(ymin = gsi - se, ymax = gsi + se), position = position_dodge(width = 0.9), width = 0.2) +
  ...
  scale_fill_manual(values = diet_colors) + labs(title = "Cape sea urchin GSI (%) per diet and temperature over time",
       x = "Temperature",
       y = "GSI (%)",
       fill = "Diet")+
  theme_classic() + theme(plot.title = element_text(hjust = 0.5))
...
```{r}
#all
sex_GSI_dat <- gonad %>% group_by(sex,diet,week) %>% 
  summarise(gsi = mean(GSI),
            se = sd(GSI)/sqrt(length(GSI))) 

#sex_GSI_dat <- inner_join(sex_GSI_dat,gonad %>% select(treatment,temperature,diet)%>% unique(),by = "diet")  
...
sex_GSI_plot <- sex_GSI_dat %>%
  ggplot(aes(x = sex, y = gsi, fill = diet)) +
 geom_col(position = position_dodge(width = 0.9), color = "black", width = 0.9) + geom_errorbar(aes(ymin = gsi - se, ymax = gsi + se), position = position_dodge(width = 0.9), width = 0.2) +
  facet_wrap(~ week, ncol = 3, labeller = labeller(week= week.labs)) +
  scale_fill_manual(values = diet_colors) + labs(title = "Cape sea urchin GSI (%) per diet and sex over time",
       x = "Sex",
       y = "GSI (%)",
       fill = "Diet")+
  theme_classic() + theme(plot.title = element_text(hjust = 0.5))

file_path <- "C:/Users/aimee/OneDrive - University of Cape Town/OneDrive - University of Cape Town/msc_thesis/parechinus/graphs/sex_parechinus_gsi.png"

ggsave(file_path, sex_GSI_plot, width = 14, height = 6, units = "in")
...
gonad_col_L_plot <- gonad_col_dat %>%
 ...
 geom_col(position = position_dodge(width = 0.9), color = "black", width = 0.9) + geom_errorbar(aes(ymin = L - L_se, ymax = L + L_se), position = position_dodge(width = 0.9), width = 0.2) +
  facet_wrap(~ week, ncol = 3, labeller = labeller(week= week.labs)) +
  scale_fill_manual(values = diet_colors) + labs(title = "Cape sea urchin gonad lightness (CIE L*) per diet and temperature over time",
       x = "Temperature",
       y = "CIE L*(Lightness)",
       fill = "Diet")+
  theme_classic() + theme(plot.title = element_text(hjust = 0.5))
...
gonad_col_a_plot <- gonad_col_dat %>%
 ...
 geom_col(position = position_dodge(width = 0.9), color = "black", width = 0.9) + geom_errorbar(aes(ymin = a - a_se, ymax = a + a_se), position = position_dodge(width = 0.9), width = 0.2) +
  facet_wrap(~ week, ncol = 3, labeller = labeller(week= week.labs)) +
  scale_fill_manual(values = diet_colors) + labs(title = "Cape sea urchin gonad redness (CIE a*) per diet and temperature over time",
       x = "Temperature",
       y = "CIE a*(Redness)",
       fill = "Diet")+
  theme_classic() + theme(plot.title = element_text(hjust = 0.5))
...
gonad_col_b_plot <- gonad_col_dat %>%
 ...
 geom_col(position = position_dodge(width = 0.9), color = "black", width = 0.9) + geom_errorbar(aes(ymin = b - b_se, ymax = b + b_se), position = position_dodge(width = 0.9), width = 0.2) +
  facet_wrap(~ week, ncol = 3, labeller = labeller(week= week.labs)) +
  scale_fill_manual(values = diet_colors) + labs(title = "Cape sea urchin gonad yellowness (CIE b*) per diet and temperature over time",
       x = "Temperature",
       y = "CIE b*(Yellowness)",
       fill = "Diet")+
  theme_classic() + theme(plot.title = element_text(hjust = 0.5))
...
Plot gonad colour by sex: 
- Diet + sex
```{r}
#all
gonad_col_sex_dat <- gonad %>% group_by(sex,diet,week) %>% 
  summarise(L = mean(`L*`),
            L_se = sd(`L*`)/sqrt(length(`L*`)), a = mean(`a*`),
            a_se = sd(`a*`)/sqrt(length(`a*`)), b = mean(`b*`),
            b_se = sd(`b*`)/sqrt(length(`b*`))) 

#gonad_col_sex_dat <- inner_join(gonad_col_sex_dat,gonad %>% select(treatment,temperature,diet)%>% unique(),by = "treatment")
...
#lightness - diet + sex
sex_gonad_col_L_plot <- gonad_col_sex_dat %>%
  ggplot(aes(x = sex, y = L, fill = diet)) +
 geom_col(position = position_dodge(width = 0.9), color = "black", width = 0.9) + geom_errorbar(aes(ymin = L - L_se, ymax = L + L_se), position = position_dodge(width = 0.9), width = 0.2) +
  facet_wrap(~ week, ncol = 3, labeller = labeller(week= week.labs)) +
  scale_fill_manual(values = diet_colors) + labs(title = "Cape sea urchin gonad lightness (CIE L*) per diet and sex over time",
       x = "Sex",
       y = "CIE L*(Lightness)",
       fill = "Diet")+
  theme_classic() + theme(plot.title = element_text(hjust = 0.5))

file_path <- "C:/Users/aimee/OneDrive - University of Cape Town/OneDrive - University of Cape Town/msc_thesis/parechinus/graphs/sex_parechinus_gonad_L.png"

ggsave(file_path, sex_gonad_col_L_plot, width = 14, height = 6, units = "in")

#redness
sex_gonad_col_a_plot <- gonad_col_sex_dat %>%
  ggplot(aes(x = sex, y = a, fill = diet)) +
 geom_col(position = position_dodge(width = 0.9), color = "black", width = 0.9) + geom_errorbar(aes(ymin = a - a_se, ymax = a + a_se), position = position_dodge(width = 0.9), width = 0.2) +
  facet_wrap(~ week, ncol = 3, labeller = labeller(week= week.labs)) +
  scale_fill_manual(values = diet_colors) + labs(title = "Cape sea urchin gonad redness (CIE a*) per diet and sex over time",
       x = "Sex",
       y = "CIE a*(Redness)",
       fill = "Diet")+
  theme_classic() + theme(plot.title = element_text(hjust = 0.5))

file_path <- "C:/Users/aimee/OneDrive - University of Cape Town/OneDrive - University of Cape Town/msc_thesis/parechinus/graphs/sex_parechinus_gonad_a.png"

ggsave(file_path, sex_gonad_col_a_plot, width = 14, height = 6, units = "in")

#yellowness
sex_gonad_col_b_plot <- gonad_col_sex_dat %>%
  ggplot(aes(x = sex, y = b, fill = diet)) +
 geom_col(position = position_dodge(width = 0.9), color = "black", width = 0.9) + geom_errorbar(aes(ymin = b - b_se, ymax = b + b_se), position = position_dodge(width = 0.9), width = 0.2) +
  facet_wrap(~ week, ncol = 3, labeller = labeller(week= week.labs)) +
  scale_fill_manual(values = diet_colors) + labs(title = "Cape sea urchin gonad yellowness (CIE b*) per diet and sex over time",
       x = "Sex",
       y = "CIE b*(Yellowness)",
       fill = "Diet")+
  theme_classic() + theme(plot.title = element_text(hjust = 0.5))

file_path <- "C:/Users/aimee/OneDrive - University of Cape Town/OneDrive - University of Cape Town/msc_thesis/parechinus/graphs/sex_parechinus_gonad_b.png"

ggsave(file_path, sex_gonad_col_b_plot, width = 14, height = 6, units = "in")
...
gonad <- gonad %>%
  filter(!(week == 26 & diet == "formulated"))


#all
gonad_col_sex_dat <- gonad %>% group_by(sex,week) %>% 
  summarise(L = mean(`L*`),
            L_se = sd(`L*`)/sqrt(length(`L*`)), a = mean(`a*`),
            a_se = sd(`a*`)/sqrt(length(`a*`)), b = mean(`b*`),
            b_se = sd(`b*`)/sqrt(length(`b*`))) 
...
sex_colors <- c("female" = "pink", "male" = "lightblue")


#lightness - diet + sex
sex_gonad_col_L_plot2 <- gonad_col_sex_dat %>%
  ggplot(aes(x = sex, y = L, fill = sex)) +
 geom_col(position = position_dodge(width = 0.9), color = "black", width = 0.9) + geom_errorbar(aes(ymin = L - L_se, ymax = L + L_se), position = position_dodge(width = 0.9), width = 0.2) +
  facet_wrap(~ week, ncol = 3, labeller = labeller(week= week.labs)) +
  scale_fill_manual(values = sex_colors) + labs(title = "Cape sea urchin gonad lightness (CIE L*) per sex over time",
       x = "Sex",
       y = "CIE L*(Lightness)",
       fill = "Sex")+
  theme_classic() + theme(plot.title = element_text(hjust = 0.5))

file_path <- "C:/Users/aimee/OneDrive - University of Cape Town/OneDrive - University of Cape Town/msc_thesis/parechinus/graphs/sex_only_gonad_L.png"

ggsave(file_path, sex_gonad_col_L_plot2, width = 14, height = 6, units = "in")

#redness
sex_gonad_col_a_plot2 <- gonad_col_sex_dat %>%
  ggplot(aes(x = sex, y = a, fill = sex)) +
 geom_col(position = position_dodge(width = 0.9), color = "black", width = 0.9) + geom_errorbar(aes(ymin = a - a_se, ymax = a + a_se), position = position_dodge(width = 0.9), width = 0.2) +
  facet_wrap(~ week, ncol = 3, labeller = labeller(week= week.labs)) +
  scale_fill_manual(values = sex_colors) + labs(title = "Cape sea urchin gonad redness (CIE a*) per sex over time",
       x = "Sex",
       y = "CIE a*(Redness)",
       fill = "Sex")+
  theme_classic() + theme(plot.title = element_text(hjust = 0.5))

file_path <- "C:/Users/aimee/OneDrive - University of Cape Town/OneDrive - University of Cape Town/msc_thesis/parechinus/graphs/sex_only_gonad_a.png"

ggsave(file_path, sex_gonad_col_a_plot2, width = 14, height = 6, units = "in")

#yellowness
sex_gonad_col_b_plot2 <- gonad_col_sex_dat %>%
  ggplot(aes(x = sex, y = b, fill = sex)) +
 geom_col(position = position_dodge(width = 0.9), color = "black", width = 0.9) + geom_errorbar(aes(ymin = b - b_se, ymax = b + b_se), position = position_dodge(width = 0.9), width = 0.2) +
  facet_wrap(~ week, ncol = 3, labeller = labeller(week= week.labs)) +
  scale_fill_manual(values = sex_colors) + labs(title = "Cape sea urchin gonad yellowness (CIE b*) per sex over time",
       x = "Sex",
       y = "CIE b*(Yellowness)",
       fill = "Sex")+
  theme_classic() + theme(plot.title = element_text(hjust = 0.5))

file_path <- "C:/Users/aimee/OneDrive - University of Cape Town/OneDrive - University of Cape Town/msc_thesis/parechinus/graphs/sex_only_gonad_b.png"

ggsave(file_path, sex_gonad_col_b_plot2, width = 14, height = 6, units = "in")
...
gonad_stage_plot <- stage_dat %>%
 ...
 geom_col(position = position_dodge(width = 0.9), color = "black", width = 0.9) + geom_errorbar(aes(ymin = gstage - se, ymax = gstage + se), position = position_dodge(width = 0.9), width = 0.2) +
  facet_wrap(~ week, ncol = 3, labeller = labeller(week= week.labs)) +
  scale_fill_manual(values = diet_colors) + labs(title = "Cape sea urchin gonad stage per diet and temperature over time",
       x = "Temperature",
       y = "Gonad stage",
       fill = "Diet")+
  theme_classic() + theme(plot.title = element_text(hjust = 0.5))
...
feed_dat <- feed %>% group_by(treatment,week) %>% 
  summarise(feedcon = mean(`feed consumed per animal per week` ),
            se = sd(`feed consumed per animal per week`)/sqrt(length(`feed consumed per animal per week`))) 
...
feed_plot <- feed_dat %>%
  ggplot(aes(x = temperature, y = feedcon, fill = diet)) +
  geom_col(position = position_dodge(width = 0.9), color = "black", width = 0.9) +
  geom_errorbar(aes(ymin = feedcon - se, ymax = feedcon + se), position = position_dodge(width = 0.9), width = 0.2) +
  facet_wrap(~ week, scales = "free_x", ncol = 4) +
  scale_fill_manual(values = diet_colors) + labs(title = "Cape sea urchin feed consumption rate (feed consumed per animal per week) per diet and temperature over time",
       x = "Week",
       y = "Feed consumption rate (feed consumed per animal per week)",
       color = "Diet")+
  theme_classic() + theme(plot.title = element_text(hjust = 0.5))
...
ggsave(file_path, feed_plot, width = 14, height = 6, units = "in")
```
...
temp_colors <- c("Ambient" = "cornflowerblue", "Warmed" = "brown")

somatic_dates <- c("2023-06-09","2023-07-07", "2023-08-04", "2023-09-04", "2023-10-09", "2023-11-13")

gonad_dates <- c("2023-08-10", "2023-10-17", "2023-12-05")

datacollection_dates <- data.frame(somatic_dates, Lines = "somatic data collection")
gonadcollection_dates <- data.frame(gonad_dates, Lines = "gonad data collection")

datacollection_colors <- c(somatic_dates = "azure4", gonad_dates = "darkorange")

temp_plot <- temp_dat %>%
  ggplot(aes(x = Date, y = temp, color = Treatment)) +
  geom_line() +
  geom_errorbar(aes(ymin = temp - se, ymax = temp + se), width = 0.2) + geom_vline(aes(xintercept = as.Date(somatic_dates), linetype = Lines), datacollection_dates)  + geom_vline(aes(xintercept = as.Date(gonad_dates), linetype = Lines), gonadcollection_dates)+
  scale_color_manual(values = temp_colors)+ labs(title = "Temperature record (°C) for ambient and warmed treatment tanks",
       x = "Date",
       y = "Temperature (°C)",
       color = "Temperature Treatment")+  scale_x_date(date_breaks = "1 month", date_labels = "%b") +
  theme_classic() + theme(plot.title = element_text(hjust = 0.5))
...
daily_temp <- temperature %>% group_by(Date, Treatment) %>% 
  summarise(daytemp = mean(`Temp °C` ),
            se = sd(`Temp °C`)/sqrt(length(`Temp °C`)))

temp_colors <- c("Ambient" = "cornflowerblue", "Warmed" = "brown")

daily_temp %>% filter(Treatment == "Warmed") %>%  ggplot(aes(x = daytemp, color = Treatment)) +
  geom_histogram(fill = "white", position = "identity", alpha = 0.5, bins = 30) +
  labs(title = "Histogram of Temperature records", x = "Value", y = "Frequency") +
  theme_classic() +
