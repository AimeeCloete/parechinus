...
gonad26nof <- gonad26 %>% filter(diet != "formulated")
...
shapiro.test(gonad26nof$GSI) #normal
leveneTest(gonad26nof$GSI ~ gonad26nof$sex) #equal variance
t.test(GSI~sex, data = gonad26nof, var.equal=T)
...
gonad26nof %>% group_by(sex) %>% summarise(mean = mean(GSI), se = sd(GSI)/sqrt(length(GSI)))
